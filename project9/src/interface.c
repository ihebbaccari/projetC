...
create_window4 (void)
{
  ...
  la56 = gtk_label_new (_("Suppression Terminer !"));
  gtk_widget_show (la56);
  gtk_fixed_put (GTK_FIXED (fixed5), la56, 80, 48);
  gtk_widget_set_size_request (la56, 192, 41);

  bo = gtk_button_new_with_mnemonic (_("Ok"));
  gtk_widget_show (bo);
  gtk_fixed_put (GTK_FIXED (fixed5), bo, 128, 104);
  gtk_widget_set_size_request (bo, 66, 29);

  g_signal_connect ((gpointer) bo, "clicked",
                    G_CALLBACK (on_bo_clicked),
                    NULL);

  /* Store pointers to all widgets, for use by lookup_widget(). */
  GLADE_HOOKUP_OBJECT_NO_REF (window4, window4, "window4");
  GLADE_HOOKUP_OBJECT (window4, fixed5, "fixed5");
  GLADE_HOOKUP_OBJECT (window4, la56, "la56");
  GLADE_HOOKUP_OBJECT (window4, bo, "bo");
